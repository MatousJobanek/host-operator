...
test-e2e-local: set-e2e-local-path test-e2e

.PHONY: set-e2e-local-path
set-e2e-local-path:
	# set e2e repo path to ../toolchain-e2e
	$(eval E2E_REPO_PATH = ${PWD}/../toolchain-e2e)
...
ifeq ($(E2E_REPO_PATH),"")
	...
	rm -rf ${E2E_REPO_PATH}
	# clone
	git clone https://github.com/codeready-toolchain/toolchain-e2e.git --no-tags --depth 1 ${E2E_REPO_PATH}
	# check if CLONEREFS_OPTIONS is set (which would mean that it is running in OpenShift CI)
    ifneq ($(CLONEREFS_OPTIONS),)
        ...
        ifneq ($(EXISTS),"")
			# the branch with the same name exists so use it
			...
			# the branch with the same name does not exists so use master
        endif
    endif
endif
